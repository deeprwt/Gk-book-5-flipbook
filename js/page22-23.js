$(function(){

$('#_idContainer056').bind('touchstart click',function(e){
for_popup1_v('pops1','pbutton1','video1');
});

$('#_idContainer058').bind('touchstart click',function(e){
for_popup1_v('pops2','pbutton2','video2');
});

$('#_idContainer060').bind('touchstart click',function(e){
for_popup1_v('pops3','pbutton3','video3');
});

$('#_idContainer059').bind('touchstart click',function(e){
for_popup1_v('pops4','pbutton4','video4');
});

$('#_idContainer057').bind('touchstart click',function(e){
for_popup1_v('pops5','pbutton5','video5');
});

$('#_idContainer125').bind('touchstart click',function(e){
for_popup1_v('pops6','pbutton6','video6');
});

$('#_idContainer129').bind('touchstart click',function(e){
for_popup1_v('pops7','pbutton7','video7');
});

$('#_idContainer127').bind('touchstart click',function(e){
for_popup1_v('pops8','pbutton8','video8');
});

/*$('#_idContainer126').bind('touchstart click',function(e){
for_popup1_v('pops9','pbutton9','video9');
});
*/
/*$('#_idContainer128').bind('touchstart click',function(e){
for_popup1_v('pops10','pbutton10','video10');
});
*/
$('#_idContainer130').bind('touchstart click',function(e){
for_popup1_v('pops11','pbutton11','video11');
});

});